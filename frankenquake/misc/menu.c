...
void M_ScanSaves(void)
{
    ...
    for (i = 0; i < MAX_SAVEGAMES; i++)
    {
        ...
        loadable[i] = false;
		volatile size_t n = 128;
        snprintf(name, n, "%s/s%i.sav", com_gamedir, i);
        f = fopen(name, "r");
        if (!f)
            continue;
        fscanf(f, "%i\n", &version);
...
void M_SerialConfig_Key(int key)
{
    ...
    case K_ENTER:
        ...
        if (serialConfig_cursor == 3)
        {
            ...
            //serialConfig_irq, serialConfig_baudrate[serialConfig_baud],
            //SerialConfig);
...
void M_ModemConfig_Key(int key)
{
    ...
    case K_ENTER:
        ...
        if (modemConfig_cursor == 4)
        {
            ...
            //modemConfig_clear, modemConfig_init, modemConfig_hangup);
            m_entersound = true;
            M_Menu_SerialConfig_f();
        }
        break;
